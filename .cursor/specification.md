...
- **`displayQuizMode(word)`**: 평가 문제 표시
  ...
  - 정답 확인 버튼을 "정답 확인"으로 초기화하고 항상 표시
  
- **`showAnswer()`**: 정답 표시/숨기기 토글
  - 정답이 숨겨져 있을 때: 정답 표시, 가중치 +1, 버튼 텍스트를 "정답 숨기기"로 변경
  - 정답이 보여져 있을 때: 정답 숨김, 버튼 텍스트를 "정답 확인"으로 변경
...
단어 선택 및 화면 전환
  ↓
[평가하기] 가중치 기반 랜덤 선택 → 문제 표시 → 정답 확인 → 가중치 업데이트 (메모리)
  ↓
[공부하기] 테이블 표시
  ↓
