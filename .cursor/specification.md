...
- **`startStudy()`**: 학습 세션 시작
  ...
  - **평가하기 모드**: 각 단어에 대해 2개의 문제 생성 (일본어 표기, 의미)
...
1. 파일 선택 (다중 선택 가능)
2. 학습 시작
3. 각 단어에 대해 2개의 문제 생성 (일본어 표기, 의미)
4. 각 문제의 가중치를 1로 초기화
5. 가중치 기반 확률로 문제 선택: `(문제의 가중치) / (가중치의 합)`
6. 문제 표시 (문제 객체를 히스토리에 저장)
7. 정답 확인 또는 바로 넘어가기
8. 가중치 업데이트:
   - 정답 확인 후 다음: +1 (최대 5)
   - 정답 확인 없이 다음: -1 (최소 0, 가중치가 0이면 문제 완료)
9. 진행도 업데이트 (가중치가 0이 된 문제 수 / 전체 문제 수)
10. 다음 문제 선택 (5번으로)
11. **이전 버튼**: 저장된 문제 유형 그대로 표시, 가중치 변화 없음 (스택 방식)
12. **이전으로 갔다가 다시 다음**: 그 사이의 히스토리 제거 후 새 문제 추가, 가중치 변화 없음
13. **완료**: 모든 문제의 가중치가 0이 되면 알람 없이 바로 처음 화면으로 이동

### 공부하기 모드
1. 파일 선택 (단일 선택만 가능)
2. 학습 시작
3. 테이블 형태로 모든 단어 표시 (안내 문구 포함)
4. 재생 버튼(▶) 클릭 시 해당 단어의 발음 재생
5. 각 텍스트(일본어 표기, 발음, 의미) 클릭하여 개별 보임/숨김 처리
6. 컬럼 헤더 클릭하여 해당 컬럼 전체 보임/숨김 처리
7. 스크롤하여 전체 단어 확인
...
- ✅ **완료 시 자동 이동**: 모든 문제 완료 시 알람 없이 바로 처음 화면으로 이동
- ✅ **발음 재생 기능**: 공부하기 모드에서 재생 버튼(▶) 클릭 시 Web Speech API로 발음 재생 (Chrome에서 Google 일본어 음성 사용)
